document.addEventListener('DOMContentLoaded', () => {
    ...
    const createChart = (ctx, type, labels, datasets, title) => {
        ...
        const chartOptions = {
            ...
            scales: {
                x: { ticks: { color: textColor, font: { family: "'Lato', sans-serif" } }, grid: { color: gridColor } },
                y: {
                    beginAtZero: true,
                    ticks: {
                        color: textColor,
        ...
        if (type === 'bar') {
            chartOptions.plugins.tooltip.callbacks.title = () => null;
        }
    ...
    const updateCharts = (workload) => {
        ...
        document.getElementById('perf-chart-title').textContent = `Normalized Performance on ${workload.toUpperCase()}`;
        document.getElementById('hits-chart-title').textContent = `Total Prefetch Hits on ${workload.toUpperCase()}`;
